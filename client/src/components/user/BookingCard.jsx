...
const BookingCard = ({ car }) => {
  ...
  const {fetchCarDetails, carDetails} = useContext(AppContext)

  useEffect(()=>{
    fetchCarDetails(car.car)
  })
  return (
        ...
        <div className="flex flex-col items-start md:p-4 p-2 gap-2">
          ...
          <div className="flex flex-col items-start">
            <Label className="font-semibold text-lg">{`${
              carDetails.brand
            } ${" "} ${carDetails.model}`}</Label>
            <p className="text-gray-400 text-sm">{`${carDetails.category} ${"-"} ${
              carDetails.year
            }`}</p>
          </div>
        </div>
        <div className="flex flex-row gap-8 md:gap-16 lg:gap-24 xl:gap-32">
          <div className="flex flex-col gap-2 p-4">
            ...
            <div className="flex flex-col ">
              ...
              <p className="font-semibold pl-6">{`${(car.pickupDate).split("T")[0]} ${"-"} ${(car.returnDate).split("T")[0]}`}</p>
            </div>
